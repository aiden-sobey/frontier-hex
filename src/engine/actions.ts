...
export function validateAction(state: GameState, action: GameAction): string | null {
  ...
  switch (action.type) {
    case 'buildSettlement':
      return buildSettlement.validate(state, action);
    case 'buildRoad':
      return buildRoad.validate(state, action);
    case 'buildCity':
      return buildCity.validate(state, action);
    case 'rollDice':
      return rollDice.validate(state, action);
    case 'buyDevCard':
      return buyDevCard.validate(state, action);
    case 'playKnight':
      return playDevCard.validateKnight(state, action);
    case 'playRoadBuilding':
      return playDevCard.validateRoadBuilding(state, action);
    case 'playYearOfPlenty':
      return playDevCard.validateYearOfPlenty(state, action);
    case 'playMonopoly':
      return playDevCard.validateMonopoly(state, action);
    case 'moveRobber':
      return moveRobber.validate(state, action);
    case 'stealResource':
      return stealResource.validate(state, action);
    case 'discardResources':
      return discard.validate(state, action);
    case 'tradeBank':
      return trade.validateTradeBank(state, action);
    case 'tradeOffer':
      return trade.validateTradeOffer(state, action);
    case 'tradeAccept':
      return trade.validateTradeAccept(state, action);
    case 'tradeReject':
      return trade.validateTradeReject(state, action);
    case 'tradeConfirm':
      return trade.validateTradeConfirm(state, action);
    case 'tradeCancel':
      return trade.validateTradeCancel(state, action);
    case 'endTurn':
      return endTurn.validate(state, action);
    case 'setupPlaceSettlement':
      return setupPlacement.validateSetupSettlement(state, action);
    case 'setupPlaceRoad':
      return setupPlacement.validateSetupRoad(state, action);
    default:
...
export function applyAction(state: GameState, action: GameAction): ActionResult {
  ...
  switch (action.type) {
    case 'buildSettlement':
      return buildSettlement.apply(state, action);
    case 'buildRoad':
      return buildRoad.apply(state, action);
    case 'buildCity':
      return buildCity.apply(state, action);
    case 'rollDice':
      return rollDice.apply(state, action);
    case 'buyDevCard':
      return buyDevCard.apply(state, action);
    case 'playKnight':
      return playDevCard.applyKnight(state, action);
    case 'playRoadBuilding':
      return playDevCard.applyRoadBuilding(state, action);
    case 'playYearOfPlenty':
      return playDevCard.applyYearOfPlenty(state, action);
    case 'playMonopoly':
      return playDevCard.applyMonopoly(state, action);
    case 'moveRobber':
      return moveRobber.apply(state, action);
    case 'stealResource':
      return stealResource.apply(state, action);
    case 'discardResources':
      return discard.apply(state, action);
    case 'tradeBank':
      return trade.applyTradeBank(state, action);
    case 'tradeOffer':
      return trade.applyTradeOffer(state, action);
    case 'tradeAccept':
      return trade.applyTradeAccept(state, action);
    case 'tradeReject':
      return trade.applyTradeReject(state, action);
    case 'tradeConfirm':
      return trade.applyTradeConfirm(state, action);
    case 'tradeCancel':
      return trade.applyTradeCancel(state, action);
    case 'endTurn':
      return endTurn.apply(state, action);
    case 'setupPlaceSettlement':
      return setupPlacement.applySetupSettlement(state, action);
    case 'setupPlaceRoad':
      return setupPlacement.applySetupRoad(state, action);
    default:
      return { valid: false, error: 'Unknown action type', state, events: [] };
  }
}
...
export function getLegalActions(state: GameState, playerIndex: number): GameAction[] {
  ...
  switch (state.phase) {
    ...
    case GamePhase.SetupRoad: {
      ...
      for (const e of state.boardGraph.edges) {
        const ek = edgeKey(e);
        const err = setupPlacement.validateSetupRoad(state, {
          type: 'setupPlaceRoad',
          playerIndex,
          edgeKey: ek,
        });
    ...
    case GamePhase.Main: {
      ...
      if (!player.hasPlayedDevCardThisTurn) {
        ...
        if (
          player.developmentCards.includes(DevelopmentCardType.RoadBuilding) &&
          player.roads > 0
        ) {
