import {
  GameState, GamePhase, ActionResult,
  TradeBankAction, TradeOfferAction, TradeAcceptAction,
  TradeRejectAction, TradeConfirmAction, TradeCancelAction,
} from '../types'
import { cloneState, hasResources, subtractResources, addResources, totalResources } from './helpers'
...
export function applyTradeBank(state: GameState, action: TradeBankAction): ActionResult {
  ...
  return {
    valid: true,
    state: newState,
    events: [{ type: 'bankTraded', playerIndex: action.playerIndex, give: action.give, receive: action.receive, ratio }],
  }
}
...
export function applyTradeOffer(state: GameState, action: TradeOfferAction): ActionResult {
  ...
  return {
    valid: true,
    state: newState,
    events: [{ type: 'tradeOffered', playerIndex: action.playerIndex, offering: action.offering, requesting: action.requesting }],
  }
}
...
export function applyTradeConfirm(state: GameState, action: TradeConfirmAction): ActionResult {
  ...
  // Swap resources
  offerer.resources = addResources(subtractResources(offerer.resources, trade.offering), trade.requesting)
  accepter.resources = addResources(subtractResources(accepter.resources, trade.requesting), trade.offering)
  ...
  return {
    valid: true,
    state: newState,
    events: [{ type: 'tradeConfirmed', playerIndex: action.playerIndex, targetPlayerIndex: action.targetPlayerIndex }],
  }
}
