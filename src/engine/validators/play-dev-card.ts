import {
  GameState, GamePhase, DevelopmentCardType, ResourceType, ActionResult,
...
export function validateRoadBuilding(state: GameState, action: PlayRoadBuildingAction): string | null {
  if (action.playerIndex !== state.currentPlayerIndex) {
    return 'Not your turn'
  }
...
export function validateYearOfPlenty(state: GameState, action: PlayYearOfPlentyAction): string | null {
  if (action.playerIndex !== state.currentPlayerIndex) {
    return 'Not your turn'
  }
  if (state.phase !== GamePhase.Main) {
...
export function applyYearOfPlenty(state: GameState, action: PlayYearOfPlentyAction): ActionResult {
  ...
  return {
    valid: true,
    state: newState,
    events: [{ type: 'yearOfPlentyPlayed', playerIndex: action.playerIndex, resource1: action.resource1, resource2: action.resource2 }],
  }
}
...
export function applyMonopoly(state: GameState, action: PlayMonopolyAction): ActionResult {
  ...
  player.resources = { ...player.resources, [action.resource]: player.resources[action.resource] + totalStolen }
  ...
  return {
    valid: true,
    state: newState,
    events: [{ type: 'monopolyPlayed', playerIndex: action.playerIndex, resource: action.resource, totalStolen }],
  }
}
