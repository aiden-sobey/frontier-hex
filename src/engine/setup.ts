import { GameState, GamePhase, HexTile, Port, PortType, DevelopmentCardType, PlayerColor, PlayerState, AxialCoord, hexKey } from './types'
import { TERRAIN_DISTRIBUTION, NUMBER_TOKENS, DEV_CARD_DECK_COMPOSITION, EMPTY_RESOURCES, INITIAL_SETTLEMENTS, INITIAL_CITIES, INITIAL_ROADS, PORT_TYPES } from './constants'
    ...
    if (perimeterVertexKeys.has(vk1) && perimeterVertexKeys.has(vk2)) {
      ...
      if (h1 && h1.length > 0 && h2 && h2.length > 0) {
        ...
        for (const hex of boardGraph.hexes) {
          const hverts = new Set(boardGraph.hexToVertices.get(hexKey(hex))?.map(v => `${v.q},${v.r},${v.d}`))
...
const PLAYER_COLORS: PlayerColor[] = [PlayerColor.Red, PlayerColor.Blue, PlayerColor.White, PlayerColor.Orange]
