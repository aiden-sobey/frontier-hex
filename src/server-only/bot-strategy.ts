import type {
  GameState, GameAction, ResourceBundle, AxialCoord,
} from '../engine/types'
import {
  GamePhase, DevelopmentCardType, BuildingType, ResourceType, hexKey, vertexKey, edgeKey,
} from '../engine/types'
import {
  pips, RESOURCE_TYPES, TERRAIN_TO_RESOURCE,
  SETTLEMENT_COST, CITY_COST, ROAD_COST, DEV_CARD_COST, EMPTY_RESOURCES,
} from '../engine/constants'
import {
  getLegalActions, getValidSettlementLocations, getValidRoadLocations,
  getValidCityLocations, getValidRobberLocations,
} from '../engine/actions'
...
function getMostExcessResource(
  state: GameState,
  playerIndex: number,
): ResourceType | null {
...
export class BasicBotStrategy implements BotStrategy {
  ...
  private chooseDevCardPlay(state: GameState, playerIndex: number): GameAction | null {
    ...
    if (player.developmentCards.includes(DevelopmentCardType.YearOfPlenty)) {
      ...
      const needed1 = getMostNeededResource(player.resources, CITY_COST)
        ?? getMostNeededResource(player.resources, SETTLEMENT_COST)
        ?? ResourceType.Wheat
      const needed2 = getMostNeededResource(player.resources, CITY_COST)
        ?? getMostNeededResource(player.resources, SETTLEMENT_COST)
        ?? ResourceType.Ore
      return {
