...
export function useGameActions(
  ...
  const buildSettlement = useCallback(
    (vertexKey: string) =>
      dispatch({ type: 'buildSettlement', playerIndex: pi, vertexKey }),
    [dispatch, pi],
  )

  const buildRoad = useCallback(
    (edgeKey: string) =>
      dispatch({ type: 'buildRoad', playerIndex: pi, edgeKey }),
    [dispatch, pi],
  )

  const buildCity = useCallback(
    (vertexKey: string) =>
      dispatch({ type: 'buildCity', playerIndex: pi, vertexKey }),
    [dispatch, pi],
  )
  ...
  const playMonopoly = useCallback(
    (resource: ResourceType) =>
      dispatch({ type: 'playMonopoly', playerIndex: pi, resource }),
    [dispatch, pi],
  )

  const moveRobberAction = useCallback(
    (hex: AxialCoord) =>
      dispatch({ type: 'moveRobber', playerIndex: pi, hex }),
    [dispatch, pi],
  )
  ...
  const endTurn = useCallback(
    () => dispatch({ type: 'endTurn', playerIndex: pi }),
    [dispatch, pi],
  )

  const setupPlaceSettlement = useCallback(
    (vertexKey: string) =>
      dispatch({ type: 'setupPlaceSettlement', playerIndex: pi, vertexKey }),
    [dispatch, pi],
  )

  const setupPlaceRoad = useCallback(
    (edgeKey: string) =>
      dispatch({ type: 'setupPlaceRoad', playerIndex: pi, edgeKey }),
    [dispatch, pi],
  )
