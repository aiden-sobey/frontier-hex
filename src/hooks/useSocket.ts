...
interface UseSocketOptions {
  ...
  onStateUpdate?: (data: { state: ClientGameState; events: GameEvent[]; action: GameAction }) => void
...
export function useSocket(options: UseSocketOptions): UseSocketReturn {
  ...
  useEffect(() => {
    ...
    function handleStateUpdate(data: { state: ClientGameState; events: GameEvent[]; action: GameAction }) {
   ...
   */
  const sendAction = useCallback((action: GameAction): Promise<{ success: boolean; error?: string }> => {
    return new Promise((resolve) => {
      const socket = getSocket()
      if (!socket.connected) {
        resolve({ success: false, error: 'Socket not connected' })
