    ...
    <div className="flex gap-2">
      {state.players.map((player, i) => {
        ...
        const vp = getVP(
          player,
          i,
          state.longestRoadPlayer,
          state.largestArmyPlayer,
        )
        ...
        return (
          <div
            key={i}
            className={`
              border-2 rounded-lg px-3 py-2 min-w-[140px] transition-all
              ${PLAYER_COLOR_CSS[color]}
              ${isCurrentTurn ? 'ring-2 ring-yellow-400' : ''}
              ${isMe ? 'ring-1 ring-white/30' : ''}
            `}
          >
            <div className="flex items-center gap-1.5 mb-1">
              <div className={`w-2.5 h-2.5 rounded-full ${PLAYER_COLOR_DOT[color]}`} />
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-xs text-gray-300">
              ...
              <span>
                {state.longestRoadPlayer === i && (
                  <span className="text-yellow-400" title="Longest Road">LR</span>
                )}
                {state.largestArmyPlayer === i && (
                  <span className="text-yellow-400 ml-1" title="Largest Army">LA</span>
                )}
              </span>
            </div>
          </div>
